...
// `usedRecipientTypes` names which recipient types this item has already
// had a letter generated for (e.g. ['bureau'], ['bureau', 'furnisher']) —
// an item's bureau and creditor are fixed fields, so "already has a
// 'bureau' letter" always means the exact same bureau, not just a similar
// one. The Letter Generator uses this to stop a member from generating a
// second, duplicate letter to a recipient who already has one, while still
// allowing legitimate escalation-ladder progression to a *different*
// recipient type for the same item (bureau, then furnisher directly).
