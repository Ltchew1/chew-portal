...
const OPPORTUNITY_FIELDS = ['title', 'whatImproved', 'whyItMatters', 'whatItUnlocked', 'suggestedAction', 'confidence'];

// RECOMMENDATION PURITY — same fix as lib/barriers.js's upsertBarrier: a
// page view re-detecting the same active opportunity must not itself be
// a write. Read the current active row first and skip the INSERT/UPDATE
// entirely when nothing real changed, instead of running an unconditional
// ON CONFLICT DO UPDATE on every reconciliation pass.
export async function upsertOpportunity({
  userId, room, relatedGoalId, sourceKey, title, whatImproved, whyItMatters,
  whatItUnlocked, suggestedAction, confidence,
}) {
  const incoming = { title, whatImproved, whyItMatters, whatItUnlocked: whatItUnlocked ?? null, suggestedAction, confidence };
  const { rows: existingRows } = await query(
    `SELECT id, title, what_improved AS "whatImproved", why_it_matters AS "whyItMatters",
            what_it_unlocked AS "whatItUnlocked", suggested_action AS "suggestedAction",
            confidence, created_at AS "createdAt"
     FROM opportunities WHERE user_id = $1 AND source_key = $2 AND status = 'active'`,
    [userId, sourceKey]
  );
  const existing = existingRows[0];
  if (existing && OPPORTUNITY_FIELDS.every((f) => existing[f] === incoming[f])) {
