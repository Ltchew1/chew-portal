...
// Barrier / Unlock Dissolve — real state transitions only, and only on
// the exact reconciliation pass where CHEW detected them (see
// lib/intelligenceCore.js's reconcileRoom: resolvedBarriers and
// newlyActiveOpportunities are empty on every later page load, so this
// never replays an old transition as if it just happened).
//
// Deliberately two distinct outcomes, not one generic "unlocked" — a
// barrier clearing and an opportunity becoming available are different
// real facts, and the copy says which one actually happened.
//
// "PATH OPEN" (a room/pathway becoming reachable) is NOT built here:
// clientStatus and feature-flag changes are not currently logged with a
// timestamp the client can read as "just happened," so there is no real
// transition to animate — see CAPABILITY_NETWORK.md-style discipline of
// not inventing a transition the backend doesn't actually prove.
export function buildDissolveEvents(room) {
  if (!room) return [];
  const barrierEvents = (room.resolvedBarriers ?? []).map((b) => ({
    id: `barrier-${b.id}`, kind: 'barrier_cleared',
    title: b.title, whatHappened: b.whatHappened, doThisNow: b.doThisNow, resolutionNote: b.resolutionNote,
  }));
  const opportunityEvents = (room.newlyActiveOpportunities ?? []).map((o) => ({
    id: `opportunity-${o.id}`, kind: 'opportunity_unlocked',
    title: o.title, whatImproved: o.whatImproved, whyItMatters: o.whyItMatters, suggestedAction: o.suggestedAction,
  }));
  return [...barrierEvents, ...opportunityEvents];
}
