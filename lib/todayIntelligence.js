...
function buildCreditSubNodes(creditIntel, transitionEvents) {
  ...
  //
  // `nodeId` (separate from the display `id` used for React keys/DOM
  // ids elsewhere) is the cross-system matching key Cross-System Focus
  // Mode's node-level highlighting reads (app/components/today/
  // FocusController.js) — always `${entityType}:${realDatabaseId}`, the
  // same scheme lib/transitions.js's canonical events already use, so a
  // barrier/opportunity/recommendation lights up here exactly when
  // ChewMoveCard or BarrierDissolve name that same real row. null when
  // no real id backs it (never a fabricated one).
  const goal = creditIntel.goal
    ? { id: 'goal', nodeId: `goal:${creditIntel.goal.id}`, kind: 'goal', label: `Score goal: ${creditIntel.goal.targetValue}` }
    : null;
  const move = creditIntel.nextBestMove
    ? { id: 'move', nodeId: creditIntel.recommendation?.id ? `move:${creditIntel.recommendation.id}` : null, kind: 'move', label: creditIntel.nextBestMove.action }
    : null;
  const barriers = (creditIntel.activeBarriers ?? []).map((b) => ({
    id: `barrier-${b.id}`, nodeId: `barrier:${b.id}`, kind: 'barrier', label: b.title, severity: b.severity,
  }));
  ...
  const events = transitionEvents ?? buildTransitionEvents(creditIntel);
  const dissolvingBarriers = events
    .filter((e) => e.eventType === 'barrier_cleared')
    .map((e) => ({ id: `dissolving-${e.id}`, nodeId: `barrier:${e.entityId}`, kind: 'barrier', label: e.title, dissolving: true }));
  const newlyActiveIds = new Set(
    events.filter((e) => e.eventType === 'opportunity_unlocked').map((e) => e.entityId)
  );
  const opportunities = (creditIntel.activeOpportunities ?? []).map((o) => ({
    id: `opportunity-${o.id}`, nodeId: `opportunity:${o.id}`, kind: 'opportunity', label: o.title, isNew: newlyActiveIds.has(o.id),
  }));
...
export function buildLifeMapGraph({ rooms, status, isRoomLive, creditIntel, transitionEvents }) {
  const domains = buildLifeMapDomains({ rooms, status, isRoomLive, creditIntel });
  return domains.map((domain) => ({
    ...domain,
    subNodes: domain.slug === 'credit' ? buildCreditSubNodes(creditIntel, transitionEvents) : null,
  }));
}
...
export const SYSTEM_LABEL = { waiting: "What's Waiting", opportunity: "What's Opening", life_map: 'Your World', vault: 'Vault', network: 'Network' };
...
// is read off the same canonical transition events buildTransitionEvents
// produces (lib/transitions.js) — an opportunity_unlocked or
// recommendation_changed event — never re-derived from the raw room
// fields, so this can never disagree with what BarrierDissolve/Radar/
// Life Map call a transition.
//
// `transitionEvents` is optional — pass the array from a single upstream
// buildTransitionEvents(room)/buildPortalReactions(room) call to avoid
// recomputing it; omit it (most existing call sites) and this derives it
// from `room` itself, same result either way.
export function buildCrossSystemDomino(room, transitionEvents) {
  ...
  const effects = [];
  const affected = {};
  const opportunityCount = events.filter((e) => e.eventType === 'opportunity_unlocked').length;
  ...
  if (events.some((e) => e.eventType === 'recommendation_changed')) {
...
export function buildChangeStory(room, domino) {
  ...
  domino = domino ?? buildCrossSystemDomino(room);
...
// Signature Session Choreography's one real headline — reuses buildChangeStory's
// already-ranked top event where one exists (the common case: a transition is
// always reflected in whatChanged too). Falls back to the first canonical
// transition event's own title only for the narrow real gap where a
// transition landed (opportunity_unlocked/recommendation_changed) without a
// matching whatChanged entry — still a real field off a real event, never
// invented copy. Returns null (never a placeholder string) when there is
// truly nothing to lead with, which the choreography treats as "disabled."
export function pickTopChangeText(changeStory, transitionEvents) {
  if (changeStory?.headline?.text) return changeStory.headline.text;
...
// state.
//
// availableNow and newlyUnlocked are two VIEWS of the same canonical
// identity, not two independent guesses at whether they describe the
// same thing. Both read lib/opportunities.js's persisted rows — the
// same `id` a real opportunity keeps for its whole lifecycle
// (deterministic, derived from a real underlying record via sourceKey;
// see lib/homeIntelligence.js's opportunityCandidates, e.g.
// `resolved_favorable:${trackerEntryId}` or `goal_met:${goalId}` — never
// a hash of display text). newlyUnlocked's transition events carry that
// same id as `entityId` (lib/transitions.js), and availableNow excludes
// whatever just unlocked THIS pass, so one real opportunity is rendered
// as exactly one node: "newly unlocked" the pass it transitions, "active"
// on every later visit until CHEW resolves it — never both at once, and
// never split into a rolled-up summary and an individual row for the
// same underlying record (an earlier version of this function read a
// separate, unpersisted `opportunities` array off lib/homeIntelligence.js
// for this — a parallel, non-identity-bearing description of the exact
// same real facts, retired in favor of the one canonical source every
// other surface already trusts, so Radar, Life Map, and the transition
// event feed can never quietly disagree about what "this opportunity" is).
export function buildOpportunityRadar({ creditIntel, dormantRooms, transitionEvents }) {
  const events = transitionEvents ?? buildTransitionEvents(creditIntel);
  const newlyUnlocked = events.filter((e) => e.eventType === 'opportunity_unlocked');
  const newlyUnlockedIds = new Set(newlyUnlocked.map((e) => e.entityId));
  // Real, persisted, id-backed rows — the same rows Life Map's
  // opportunity subnodes and newlyUnlocked's own entityId already
  // reference (see the header comment above). Excludes anything that
  // unlocked THIS pass so the same real opportunity never renders as
  // both "active" and "newly unlocked" at once.
  const availableNow = (creditIntel?.activeOpportunities ?? [])
    .filter((o) => !newlyUnlockedIds.has(o.id))
    .map((o) => ({ id: o.id, title: o.title, whatImproved: o.whatImproved, suggestedAction: o.suggestedAction }));
