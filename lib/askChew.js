...
//
// Pure function, no DB access — easy to test, and the home page /api route
...
// Returns { matched, label, blurb, href, roomBuilt } — roomBuilt=false
// means the honest answer is "that part of your Lab isn't open yet," not a
// dead link dressed up as a feature. See app/api/home/ask/route.js for the
// one intent (score/target) currently wired to actually dispatch a write
...
export function routeAskChew(text) {
  ...
  for (const intent of INTENTS) {
    if (intent.keywords.some((kw) => q.includes(kw))) {
      const room = findRoom(intent.room);
      return {
        matched: true,
        label: intent.label,
        blurb: intent.blurb,
        href: intent.href,
        roomBuilt: room ? room.built : true,
        roomName: room?.name ?? intent.label,
