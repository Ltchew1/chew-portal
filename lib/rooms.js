...
// glance. These are real planned features, not vague marketing copy.
//
// Whether a room is actually released lives in the `features` DB table
// (feature_key `room_<slug>`, see lib/features.js), not here — that's the
// one source of truth the hub, Ask CHEW, and each room's own placeholder
// all read from, so a status change can't happen in one place and not
// another. This file stays the static, non-access-controlling metadata:
// name, icon, tagline, teaser list, and clientStatus requirement.
//
