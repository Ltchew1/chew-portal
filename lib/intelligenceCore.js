...
import { recordBarrierSnapshotIfChanged } from './frictionHistory';
...
async function reconcileRoom(clerkUserId, userId, room, intel) {
  ...
  // Friction History's snapshot layer — same architecture and purity
  // discipline as the opportunity-history snapshot below, applied to
  // barriers.id. See lib/frictionHistory.js's own comment for the
  // persistence-streak derivation this makes possible.
  const barrierHistoryResult = await recordBarrierSnapshotIfChanged({
    userId, room,
    activeBarrierIds: activeBarriers.map((b) => b.id),
    newlyDetectedIds: activeBarriers.filter((b) => b.isNew).map((b) => b.id),
  });
  const barrierHistoryChanged = barrierHistoryResult.isNew && !!barrierHistoryResult.comparison;
  ...
  return {
    ...
    opportunityHistoryChanged, barrierHistoryChanged,
  };
}
