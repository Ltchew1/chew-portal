// lib/creditRoomCompliance.js
//
// The single source of truth for every piece of compliance-load-bearing
// copy in the Credit room: the exact attestation statements a client signs
// off on, the standing disclosures, and a forbidden-phrase list that
// scripts/check-compliance-copy.js scans the whole Lab for. Nothing that
// displays this copy should hardcode its own version of it — import from
// here so there is exactly one wording to ever review or change.
//
// Why this matters architecturally: the Credit room must operate as
...
// Shown at Credit room entry (StandingDisclosures variant="entry") and
// again immediately before attestation/letter generation
// (variant="pre-generation", rendered inside AttestationGate).
...
// Phrases that would cross from "explain how it works" into "tell the
// client what to do," "CHEW performs the action for them," or "promise an
// outcome" — anywhere in The Lab, not just the Credit room (Financial
// Intelligence's future content and any other room are held to the same
// bar: real data, no guarantees). Checked by scripts/check-compliance-copy.js
// against every file under app/dashboard/lab and app/components/lab. Keep
// this list growing, not shrinking.
