...
export function buildCreditIntelligence({ items, letters, trackerEntries, untrackedLetters, address, goal, scoreSnapshots }) {
  ...
  for (const item of items) {
    if (isRecent(item.created_at)) whatChanged.push({ date: item.created_at, text: `You flagged ${item.creditor_name} (${item.bureau}).` });
  }
  for (const letter of letters) {
    if (isRecent(letter.generatedAt)) whatChanged.push({ date: letter.generatedAt, text: `You generated a letter to ${letter.recipientName}.` });
  }
  for (const entry of trackerEntries) {
    if (isRecent(entry.updatedAt)) {
      const label = entry.status === 'resolved' && entry.responseType
        ? `You logged a response from ${entry.recipientName}: ${entry.responseType.replace('_', ' ')}.`
        : entry.status === 'mailed'
          ? `You marked your letter to ${entry.recipientName} as mailed.`
          : null;
      if (label) whatChanged.push({ date: entry.updatedAt, text: label });
    }
  }
  for (const snap of scoreSnapshots) {
    if (isRecent(snap.createdAt)) whatChanged.push({ date: snap.createdAt, text: `You logged a${snap.bureau && snap.bureau !== 'overall' ? ` ${snap.bureau}` : 'n overall'} score of ${snap.score}.` });
  }
  whatChanged.sort((a, b) => new Date(b.date) - new Date(a.date));
...
export async function getCreditIntelligence(clerkUserId) {
  const [items, letters, trackerEntries, untrackedLetters, address, goal, scoreSnapshots] = await Promise.all([
  ...
  return buildCreditIntelligence({ items, letters, trackerEntries, untrackedLetters, address, goal, scoreSnapshots });
}

// Home page entry point — merges every room's intelligence into one home
// view. Today there is exactly one contributing room; a second room adds a
// getXIntelligence() here and its result joins this same array, which is
// exactly the reuse the shared "signals" primitive is for.
export async function getHomeIntelligence(clerkUserId) {
  const credit = await getCreditIntelligence(clerkUserId);
  return { rooms: [credit] };
}
