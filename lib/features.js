...
// The Admin -> Network gate — same trust model as isInternalUser() (Clerk
// publicMetadata, founder-set), used by every admin page/route directly
// rather than a feature_key, since admin access isn't a "released
// capability" with a status lifecycle — it's a role check.
export async function getAdminAccess() {
  const user = await currentUser();
  return { user, isAdmin: isInternalUser(user) };
}
