...
export async function recordScoreSnapshot({ clerkUserId, email, firstName, lastName, bureau, score, reportedDate, sourceNote }) {
  ...
  const { rows } = await query(
    `INSERT INTO credit_score_snapshots (user_id, bureau, score, reported_date, source_note)
     VALUES ($1, $2, $3, COALESCE($4, CURRENT_DATE), $5)
     RETURNING id, bureau, score, reported_date AS "reportedDate", source_note AS "sourceNote", created_at AS "createdAt"`,
    [userId, bureau ?? null, score, reportedDate ?? null, sourceNote ?? null]
  );
  return rows[0];
}
