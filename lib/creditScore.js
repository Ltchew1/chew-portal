...
import { SOURCE_TYPES, detectConflicts, describeConflictState } from './factProvenance';

const GOAL_ROOM = 'credit';
const GOAL_TYPE = 'credit_score';
const BUREAU_LABELS = { equifax: 'Equifax', experian: 'Experian', transunion: 'TransUnion', overall: 'overall' };
...
export function bureauLabel(bureau) {
  return BUREAU_LABELS[bureau] ?? bureau ?? 'overall';
}

// Full, honest history — including superseded rows — so a member (or a
// future consultant view) can always see what CHEW used to believe, not
// just what's active now. Callers that need only the "live" record
// (pickLatestScore, detectScoreConflicts) filter supersededAt themselves;
// this function never hides a row, per the Fact Conflict foundation's
// "do not delete conflict history" rule.
export async function listScoreSnapshots(clerkUserId) {
  const { rows } = await query(
    `SELECT css.id, css.bureau, css.score, css.reported_date AS "reportedDate", css.source_note AS "sourceNote",
            css.source_type AS "sourceType", css.superseded_at AS "supersededAt",
            css.superseded_reason AS "supersededReason", css.created_at AS "createdAt"
...
// per-bureau number. A superseded row (see resolveScoreConflict) is never
// "the latest" — it lost a same-context conflict, so treating it as
// current would silently un-resolve the conflict.
export function pickLatestScore(snapshots) {
  const active = snapshots.filter((s) => !s.supersededAt);
  if (active.length === 0) return null;
  return active.find((s) => s.bureau === 'overall') ?? active[0];
}

// Two snapshots are claims about the literal same real-world state only
// when they share both the bureau AND the date they claim to describe —
// see lib/factProvenance.js's detectConflicts() header for why an exact
// contextKey match (never a fuzzy date window or a numeric tolerance) is
// the whole rule: a different bureau or a different reported date is a
// different measurement, never compared. Superseded rows are excluded —
// an already-resolved conflict must not keep re-triggering.
export function detectScoreConflicts(snapshots) {
  const active = snapshots.filter((s) => !s.supersededAt);
  const observations = active.map((s) => ({
    contextKey: `${s.bureau ?? 'overall'}:${new Date(s.reportedDate).toISOString().slice(0, 10)}`,
    value: s.score,
    snapshot: s,
  }));
  return detectConflicts(observations);
}

// The one-sentence, human explanation of a single conflict group — used
// by both the Next Move "why" text and any UI surfacing the conflict, so
// the two can never describe the same contradiction differently.
export function describeScoreConflict(conflict) {
  const [{ snapshot: first }] = conflict.observations;
  const values = [...new Set(conflict.observations.map((o) => o.value))].sort((a, b) => a - b);
  return {
    bureau: first.bureau,
    bureauLabel: bureauLabel(first.bureau),
    reportedDate: first.reportedDate,
    values,
    snapshotIds: conflict.observations.map((o) => o.snapshot.id),
  };
}

// Resolving a conflict never claims a value was independently verified —
// the member is only confirming which of their OWN already-logged
// numbers is the one that's actually right; there is no path here for a
// client to grant itself a privileged source_type (reviewed_with_chew,
// connected_source, etc.) or mark anything "verified." Ownership of
// keepSnapshotId is re-checked against the DB, never trusted from a
// request body, and the function refuses to run unless a real,
// currently-unresolved conflict actually contains that snapshot — a
// client cannot use this to arbitrarily bury an unrelated score entry.
export async function resolveScoreConflict({ clerkUserId, keepSnapshotId }) {
  return withTransaction(async (client) => {
    const userRow = await client.query('SELECT id FROM users WHERE clerk_user_id = $1', [clerkUserId]);
    const userId = userRow.rows[0]?.id;
    if (!userId) throw new Error('That score entry could not be found on this account.');

    const kept = await client.query(
      `SELECT id, bureau, reported_date AS "reportedDate", score
       FROM credit_score_snapshots WHERE id = $1 AND user_id = $2 AND superseded_at IS NULL FOR UPDATE`,
      [keepSnapshotId, userId]
    );
    if (!kept.rows[0]) throw new Error('That score entry could not be found on this account.');
    const { bureau, reportedDate } = kept.rows[0];

    const group = await client.query(
      `SELECT id, score FROM credit_score_snapshots
       WHERE user_id = $1 AND bureau IS NOT DISTINCT FROM $2 AND reported_date = $3 AND superseded_at IS NULL
       FOR UPDATE`,
      [userId, bureau, reportedDate]
    );
    const distinctValues = new Set(group.rows.map((r) => r.score));
    if (distinctValues.size <= 1) {
      throw new Error('This entry is not part of an active conflict — nothing to resolve.');
    }

    const supersededIds = group.rows.map((r) => r.id).filter((id) => id !== keepSnapshotId);
    await client.query(
      `UPDATE credit_score_snapshots SET superseded_at = now(),
         superseded_reason = $2
       WHERE id = ANY($1::bigint[])`,
      [supersededIds, `Superseded when the ${bureauLabel(bureau)} score for ${new Date(reportedDate).toLocaleDateString()} was clarified.`]
    );

    await logEvent({
      client, userId, room: 'credit', eventType: 'score_conflict_resolved',
      subject: `${bureauLabel(bureau)} score for ${new Date(reportedDate).toLocaleDateString()}`,
      newState: { keptSnapshotId: keepSnapshotId, supersededIds },
      severity: 'info',
    });

    return { keptSnapshotId: keepSnapshotId, supersededIds };
  });
}
