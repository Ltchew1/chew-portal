...
export function composeDisputeLetter({ member, items, recipientType, recipientName, recipientAddressLines, escalationNotes }) {
  ...
  const escalationParagraph = escalationNotes || null;
  ...
  const lines = [
    ...
    itemLines,
    '',
    [legalParagraph, movParagraph, escalationParagraph ? `For your reference: ${escalationParagraph}` : null].filter(Boolean).join('\n\n'),
    '',
    closing,
    '',
    'Sincerely,',
