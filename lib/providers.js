...
// Partial update — the Admin -> Network workflow this exists for is
// exactly "fill in one more readiness field, flip status once the
// checklist is done," not replacing the whole row every time. Only
// classification is re-validated; every other field is COALESCE'd so an
...
export async function updateProvider(providerId, fields) {
  ...
  const { rows } = await query(
    `UPDATE providers SET
       ...
       service_status = COALESCE($4, service_status), jurisdiction = COALESCE($5, jurisdiction),
       licensing_note = COALESCE($6, licensing_note), contact_method = COALESCE($7, contact_method),
       intake_process = COALESCE($8, intake_process), disclosure_text = COALESCE($9, disclosure_text),
       data_sharing_notes = COALESCE($10, data_sharing_notes), escalation_process = COALESCE($11, escalation_process),
       updated_at = now()
     WHERE id = $1
     RETURNING id, name, classification, service_status AS "serviceStatus", jurisdiction,
               licensing_note AS "licensingNote", contact_method AS "contactMethod",
               intake_process AS "intakeProcess", disclosure_text AS "disclosureText",
               data_sharing_notes AS "dataSharingNotes", escalation_process AS "escalationProcess"`,
    [providerId, name ?? null, classification ?? null, serviceStatus ?? null, jurisdiction ?? null,
      licensingNote ?? null, contactMethod ?? null, intakeProcess ?? null, disclosureText ?? null,
      dataSharingNotes ?? null, escalationProcess ?? null]
  );
