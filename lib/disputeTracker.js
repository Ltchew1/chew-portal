...
// The one real source of "how much time has actually passed" and "what
// response actually came back" for a given letter — client-self-reported,
// never inferred from generated_at (a letter can sit generated for days
// before it's actually mailed; see this file's header comment). Used by
// the escalation gate in lib/letters.js so a CFPB/FTC complaint can never
// assert a timing or response fact the client hasn't actually logged.
export async function getTrackerEntryForLetter(clerkUserId, letterId) {
  const { rows } = await query(
    `SELECT dte.id, dte.mailed_date AS "mailedDate", dte.status,
            dte.response_type AS "responseType", dte.response_date AS "responseDate"
     FROM dispute_tracker_entries dte
     JOIN users u ON u.id = dte.user_id
     WHERE u.clerk_user_id = $1 AND dte.letter_id = $2`,
