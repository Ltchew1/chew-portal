  ...
  const { rows } = await query(
    `INSERT INTO provider_handoffs
       (user_id, capability_id, provider_id, related_goal_id, fields_disclosed, status,
        need_type, reason, urgency, origin_event_id)
     SELECT $1, c.id, $2, $3, $4, 'consent_pending', $6, $7, $8, $9
     FROM capabilities c WHERE c.key = $5
     RETURNING id, capability_id AS "capabilityId", provider_id AS "providerId", status,
               fields_disclosed AS "fieldsDisclosed", need_type AS "needType", reason, urgency,
               created_at AS "createdAt"`,
    [userId, providerId, relatedGoalId ?? null, JSON.stringify(fieldsDisclosed), capabilityKey,
      needType ?? null, reason ?? null, urgency ?? 'normal', originEventId ?? null]
  );
...
export async function recordOutcome(clerkUserId, handoffId, outcomeNote) {
  ...
  // Closes the loop precisely: this handoff row now points at the exact
  // event its outcome created, so anything reading the event stream (a
  // future recommendation recalculation) can trace back to which handoff
  // caused it, not just that "something happened."
