...
// Admin -> Network's cross-user event trail — not scoped to one client,
// since staff need to reconstruct what happened across the whole network
// room, not just one person's timeline.
export async function listAllEvents({ room, limit = 50 } = {}) {
  const params = [];
  let roomClause = '';
  if (room) {
    params.push(room);
    roomClause = `WHERE ce.room = $${params.length}`;
  }
  params.push(limit);
  const { rows } = await query(
    `SELECT ce.id, u.clerk_user_id AS "clerkUserId", ce.room, ce.event_type AS "eventType", ce.subject,
            ce.occurred_at AS "occurredAt", ce.source, ce.severity, ce.requires_action AS "requiresAction", ce.metadata
     FROM chew_events ce
     JOIN users u ON u.id = ce.user_id
     ${roomClause}
