...
// ============================================================================
// TWO LOCKED FUTURE-ARCHITECTURE RULES (recorded, not yet exercised by any
// code — the Credit score is still this file's only real domain):
//
// 1. COMPARABILITY BEFORE CONFLICT. The Credit score's contextKey today is
//    (bureau, reported_date) — accepted ONLY for this narrow data model,
//    never assumed to be CHEW's permanent definition of "comparable credit
//    score." A future observation (a different scoring model/version, a
//    different score type, a document-derived value) may need more
//    dimensions folded into its contextKey before two values are even
//    eligible to be evaluated as contradictory. Different values are NOT
//    automatically conflict — they're only a candidate for detectConflicts()
//    once the caller has established the two observations genuinely claim
//    the same thing. Get the contextKey right; detectConflicts() itself
//    never needs to change.
//
// 2. RESOLUTION DOES NOT CHANGE PROVENANCE. Today's only resolution
//    (lib/creditScore.js's resolveScoreConflict) is between two
//    MEMBER_PROVIDED observations — a member confirming which of their own
//    numbers is right is CLARIFICATION/SUPERSESSION, never VERIFICATION.
//    A future resolution between observations of DIFFERENT source types
//    (a member-provided value vs. a document-provided one, say) must still
//    preserve each observation's own original sourceType — choosing which
//    value wins must never silently promote the loser or the winner into a
//    more-trusted source class than it actually earned.
// ============================================================================
