...
//      app/api/credit-lab/** route files — those routes may only ever
//      talk to our own Postgres via lib/db.js.
...
function main() {
  ...
  const creditLabApiFiles = allFiles.filter((f) =>
    f.includes(`${path.sep}app${path.sep}api${path.sep}credit-lab${path.sep}`)
  );

  let violations = 0;

  for (const file of creditLabApiFiles) {
    const lines = fs.readFileSync(file, 'utf8').split('\n');
    lines.forEach((line, i) => {
      if (NETWORK_CALL_PATTERN.test(line)) {
        console.error(
          `${path.relative(REPO_ROOT, file)}:${i + 1}: outbound network call in a Credit Lab API route — ` +
  ...
  console.log(
    `Checked ${creditLabApiFiles.length} Credit Lab API route file(s) for outbound calls, and ` +
